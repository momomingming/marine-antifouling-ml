...
> ⚠️ **口径更正（2026-09-19）**：下方表格中「XGBoost 盲测 R² = 0.70~0.75」仍来自
> **分层随机抽样**切分——同一分子的增强变体会同时落入训练集与盲测集，存在**信息泄漏**，
> 绝对数值偏乐观。本节的判断（KNN 0.97 虚高、XGBoost 更稳）**方向正确**，
> 但 XGBoost 的 0.72 亦不可对外宣称。现行统一口径为
> **Leave-One-Group-Out by SMILES 池化折外 R² = 0.592 / MAE 4.46**（防污效率）。
> 本节的价值在于**同一协议下的模型横向排序**，而非绝对性能数字。
